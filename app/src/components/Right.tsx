  ...
  if (props.leftArray[0]) {
    if(Array.isArray(props.handlerInfo[props.selectedTab].serverResponse)) {
      props.handlerInfo[props.selectedTab].serverResponse.forEach( message => {
        ...
        if(message.type ==='write') clientJSON.push(message.payload.payload);
      })
    } else {
  ...
  return (
    <div className="right-half">
      <div className={"right-header-box"}>
        <h2 className={serverTabClass} onClick={() => selectResponseTab({selectedTab: selectedTab, mode: "server"})} >SERVER RESPONSES</h2>
        <h2 className={clientTabClass} onClick={() => selectResponseTab({selectedTab: selectedTab, mode: "client"})} >CLIENT MESSAGES</h2>
      </div>
      <div className="response-display">
        <ReactJson theme={"monokai"} iconStyle={"circle"} indentWidth={2} displayDataTypes={false} src={displayedJSON} />
      </div>
