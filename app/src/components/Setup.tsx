      ...
      field.forEach( value => {
        if (typeof value === 'object' && !Array.isArray(value)) {
          ...
          if (label === "REPEATED" && type === value.typeName) {
            let repeatedElement = generateFields(props.messageList[type].type.field, type, depth + 1)
            elementsArray.push(
              <ul>
                {/* <h3>{messageName}</h3> */}
                <li className="first">
            ...
            elementsArray.push(
              <ul>
                {/* <h3>{messageName}</h3> */}
                <li className="first">
                  <button className="setup-button singular" disabled={label === 'REPEATED' ? false : true}>{label === 'REPEATED' ? '+' : ''}</button>
                  <div className="setup-name">{name}</div>
                  <div className="setup-label">{label}</div>
                  <div className="setup-type">{type}</div>
